...
# Ensure the date column is a proper datetime and create a date-only column for safe comparisons
# (This avoids TypeError comparing pandas datetime64 with python date or strings)
df["date"] = pd.to_datetime(df["date"], errors="coerce")
# .dt.date converts to python date objects (or NaT -> NaT). This makes comparisons with
# Streamlit date_input values (which are datetime.date) safe and consistent.
...
# Use date_only (python date objects) when computing min/max and when presenting date_input
date_min = df["date_only"].dropna().min() if df["date_only"].notna().any() else None
date_max = df["date_only"].dropna().max() if df["date_only"].notna().any() else None
...
if start_date is not None and end_date is not None:
    try:
        # Compare the date-only python dates to the selected start/end (also python dates)
        f = f[(f["date_only"] >= start_date) & (f["date_only"] <= end_date)]
    except Exception:
        # Fallback: try converting start/end to pandas Timestamps and compare to full datetime column
        try:
            start_ts = pd.to_datetime(start_date)
            end_ts = pd.to_datetime(end_date)
            f = f[(f["date"] >= start_ts) & (f["date"] <= end_ts)]
        except Exception:
            # If anything unexpected happens, skip date filtering (safer than crashing the app)
            st.warning("Date filtering could not be applied due to incompatible date formats.")
            pass
if sel_sent:
